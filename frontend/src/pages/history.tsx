  ...
  return (
    <div className="h-full w-full bg-[#E5E7EB] overflow-hidden font-['Inter',system-ui,sans-serif] select-text">
      ...
      <div className="max-w-[1000px] mx-auto relative z-10 h-full flex flex-col pt-8 px-8 items-center">
        ...
        {/* HEADER */}
        <header className={`w-full flex flex-col md:flex-row justify-between items-start md:items-end gap-4 md:gap-0 mb-8 md:mb-10 px-4 md:px-8 animate-[popIn_0.4s_ease-out_forwards] relative mt-12 md:mt-0 ${isSortOpen ? 'z-[200]' : 'z-[100]'}`}>
        ...
        <div className="w-full flex-1 overflow-y-auto no-scrollbar pb-40 px-4 md:px-16 md:-mx-16">
          {processedData.length > 0 && (
            <div className="hidden md:grid grid-cols-[1fr_180px_40px] px-8 mb-5 items-center opacity-60">
            ...
            {isLoading ? (
              ...
              processedData.map((item, index) => (
                <div
                  ...
                  className={`w-full group flex flex-col md:grid md:grid-cols-[1fr_180px_40px] items-start md:items-center gap-3 md:gap-0 px-5 md:px-8 py-4 bg-white/40 backdrop-blur-xl border border-white/60 rounded-[24px] transition-all duration-500 cursor-pointer relative animate-[popIn_0.4s_ease-out_forwards] shadow-[0_4px_20px_rgba(0,0,0,0.02)] z-10 hover:z-[40] hover:-translate-y-2 hover:scale-[1.01] hover:shadow-xl hover:bg-white/60 hover:border-white/90`}
                  style={{ animationDelay: `${index * 0.04}s` }}
                >
                  {/* Field Info */}
                  <div className="flex items-center gap-4 md:gap-5 overflow-visible pointer-events-none min-w-0 w-full">
                    ...
                    <div className="flex flex-col min-w-0 items-start w-full">
                      ...
                      <div className="flex justify-between items-center w-full md:w-auto">
                          ...
                          <div className="md:hidden text-right text-[9px] font-bold text-[#6B7280] font-['JetBrains_Mono',monospace] mt-1 ml-2">
                  ...
                  <div className="hidden md:block text-right text-[10px] font-bold text-[#6B7280] font-['JetBrains_Mono',monospace] pr-4 pointer-events-none">
                    {formatDate(item.createdAt).toUpperCase()}
                  </div>
